...
void cleanWorldPos(sworld world_from, sworld world_to, int x_from, int y_from,int worldsize){
	cleanPos(world_from, x_from, y_from, worldsize);
	cleanPos(world_to, x_from, y_from, worldsize);
}

void checkBabies(sworld world_from, sworld world_to, sworld animalAux, int x_from, int y_from, int worldsize) {
...
void moveWOLFs(sworld world_from, sworld world_to, sworld animalAuxFrom,
		sworld animalAuxTo, int x_from, int y_from, int x_to, int y_to, int worldsize) {
	if (animalAuxTo->type == WOLF || WES) {
		...
		if(animalAuxFrom->starvation_period > animalAuxTo->starvation_period){
			setPosition(world_to, x_to, y_to, type, animalAuxFrom->breeding_period,animalAuxFrom->starvation_period, worldsize);
		}
		else{
			if(animalAuxFrom->starvation_period < animalAuxTo->starvation_period){
				setPosition(world_to, x_to, y_to, type, animalAuxTo->breeding_period,animalAuxTo->starvation_period, worldsize);
			}
			else{
				if(animalAuxFrom->starvation_period == animalAuxTo->starvation_period){
				/*Breeding Maior*/
		...
		if (animalAuxTo->type == SQRL) {
			if (animalAuxFrom->type == WES) {
				...
				setPosition(world_to, x_to, y_to, WES, animalAuxFrom->breeding_period,
						animalAuxFrom->starvation_period, worldsize);
			}
	...
	checkBabies(world_from,world_to,animalAuxFrom, x_from, y_from, worldsize);
...
void moveSQRLs(sworld world_from, sworld world_to, sworld animalAuxFrom,
	...
	if (animalAuxTo->type == WOLF || WES) { /*Animal to is in world_to*/
		...
		if (animalAuxTo->type == WES) {
			...
			setPosition(world_to, x_to, y_to, WES, animalAuxTo->breeding_period,
					animalAuxTo->starvation_period, worldsize);
		}
	/*	cleanPos(world_from, x_to, y_to); *//*o esquilo é comido e limpa a posição*/ /*?*/
		...
		if (animalAuxTo->type == SQRL || animalAuxTo->type == SONT) {
			/*Breeding Maior*/
			if(animalAuxFrom->breeding_period > animalAuxTo->breeding_period){
				setPosition(world_to, x_to, y_to, animalAuxTo->type, animalAuxFrom->breeding_period,animalAuxFrom->starvation_period, worldsize); /*to type pq pode ser SONT*/
			}
			else{
				if(animalAuxFrom->breeding_period <= animalAuxTo->breeding_period){
					setPosition(world_to, x_to, y_to, animalAuxTo->type, animalAuxTo->breeding_period,animalAuxTo->starvation_period, worldsize); /*to type pq pode ser SONT*/
				}
				else{
					printf("BUG stupid BUG");
				}
...
void move(sworld world_from, int x_from, int y_from, sworld world_to, int x_to,
	...
	void calcCords(int pos, int* x, int* y, int worldsize) {
		int aux1 = 0, aux2 = 0;
		aux1 = pos / worldsize;
		(*x) = aux1;
